...
const VehicleDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkCashRegister = async () => {
      ...
      try {
        // Obtener caja actual
        const currentRes = await fetch('/api/cash-register/current', { headers });
        ...
        const historyRes = await fetch('/api/cash-register/history', { headers });
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <body>
        <h1>${user?.nombreTaller || 'Taller Mecánico'}</h1>
        ...
        <div class="footer">
          Documento generado el ${new Date().toLocaleDateString()} - ${user?.nombreTaller || 'Taller Mecánico'}
        </div>
      </body>
      </html>
    `;
