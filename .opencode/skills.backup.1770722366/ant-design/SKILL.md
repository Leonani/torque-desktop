...
name: ant-design
description: >
  Ant Design ecosystem guidance covering antd 6.x, Ant Design Pro 5/ProComponents, and Ant Design X v2 (AI/chat UI).
  Trigger: When making component/layout decisions, theming/tokens, SSR, routing/access, CRUD patterns, or AI chat UI integrations.
license: Apache-2.0
metadata:
  author: Jose Limardo
  version: "1.0"
  scope: [root, ui]
  auto_invoke: "Using Ant Design Pro or Ant Design X"
---
...
- Language: TypeScript.
- Styling: prefer tokens and `classNames` wired to local CSS Modules; avoid inline styles (including `styles` props) and global overrides.
...
- Do not use static feedback APIs (they do not inherit context/theme/locale):
  - Forbidden: `message.success()`, `notification.open()`, `Modal.confirm()`.
  - Required: hook-based APIs `message.useMessage()`, `notification.useNotification()`, `Modal.useModal()` (or `App.useApp()` if your app uses `App`).
  - Render the returned `contextHolder` once near the app/root layout.
...
- Feedback (message/notification/modal): prefer hook-based APIs to preserve `ConfigProvider` context; avoid static methods.
...
2. Use component tokens or `classNames` (mapped to CSS Modules) for differences.
3. Only if unavoidable, use scoped CSS overrides and state the risk.
4. Never rely on global `.ant-*` overrides.

### Minimal pattern: hook-based feedback APIs
...
export function Page() {
  ...
  return (
    <>
      ...
      {/* modalApi.confirm({ title: "Confirm" }) */}
      {/* notificationApi.info({ message: "Heads up" }) */}
    </>
  );
}
```
...
ALL Ant Design components and layouts MUST be responsive:

- **Mobile-first**: Start with mobile layouts, then enhance for larger screens.
- **Grid system**: Use AntD's `Row` and `Col` components for responsive layouts.
- **Breakpoints**: xs (<576px), sm (≥576px), md (≥768px), lg (≥992px), xl (≥1200px), xxl (≥1600px).
- **Adaptive components**: Tables, Forms, Modals, and Drawers must adapt to screen size.
- **Touch targets**: Minimum 44px touch targets on mobile.
- **Collapsible**: Use `Layout.Sider` collapsible for mobile navigation.
- **Hidden elements**: Use responsive props or `Flex` to show/hide based on breakpoints.
...
- **Tables**: Use `scroll={{ x: true }}` for responsive tables, consider `size="small"` on mobile.
- **Modals**: Use `width` with percentage or responsive breakpoints.
- **Menus**: Use hamburger menu pattern on mobile (see `pro-layout-advanced.md`).

### Regression checklist additions

- [ ] **Responsive**: Grid system (`Row`/`Col`) used correctly with appropriate breakpoints.
- [ ] **Mobile**: Touch targets ≥44px; collapsible menus on mobile.
- [ ] **Tables**: Responsive scroll (`scroll={{ x: true }}`) when needed.
- [ ] **Modals**: Widths adapt to viewport; no horizontal scrolling.
