...
**Tests MUST be in `src/__tests__/` mirroring the `src/` directory structure:**
...
1. **Location**: All tests MUST be in `src/__tests__/` (NOT co-located with source files)
2. **Naming**: `ComponentName.test.tsx` or `utilityName.test.ts`
3. **Structure**: Mirror the `src/` folder hierarchy exactly
4. **Imports**: Use path aliases (`@/`, `@components/`, etc.) - NEVER relative imports like `../../`
...
├── components/
│   ├── Button.tsx
│   └── Button.test.tsx          ← WRONG: Co-located test
```
...
└── __tests__/
    └── components/
        └── Button.test.tsx      ← CORRECT: In __tests__
```
