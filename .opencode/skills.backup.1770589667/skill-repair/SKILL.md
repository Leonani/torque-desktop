...
file docs/skills/*/SKILL.md | grep CRLF

# Or check with od command
od -c docs/skills/skill-name/SKILL.md | head -n 5 | grep '\\r'
```
...
```bash
sed -i 's/\r$//' docs/skills/skill-name/SKILL.md
```
...
#!/bin/bash
source docs/skills/skill-sync/assets/sync.sh 2>/dev/null
...
bash test_metadata.sh docs/skills/skill-name/SKILL.md
```
...
bash docs/skills/skill-repair/assets/repair.sh
```
...
bash docs/skills/skill-sync/assets/sync.sh
...
# Fix all skills
find docs/skills -name "SKILL.md" -type f -exec sed -i 's/\r$//' {} \;
```
...
```bash
#!/bin/bash
# Repair all SKILL.md files in docs/skills/
...
- **Sync Script**: [docs/skills/skill-sync/assets/sync.sh](../skill-sync/assets/sync.sh)
